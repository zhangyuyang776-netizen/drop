...
def test_sparse_fd_setvalues_only_local_rows(tmp_path: Path):
    ...
    from assembly import build_sparse_fd_jacobian as fdmod  # noqa: E402
    ...
    try:
        P_dm = mgr.dm.createMatrix()
    except Exception:
        P_dm = None
    P, _extras = fdmod.build_sparse_fd_jacobian(
        ctx,
    ...
    orig_setvalues = fdmod._mat_setvalues_local

    def set_values_checked(mat, rows, cols, values, *args, **kwargs):
        rows_arr = np.atleast_1d(np.asarray(rows, dtype=np.int64))
        if rows_arr.size:
            if (rows_arr < rstart).any() or (rows_arr >= rend).any():
                raise AssertionError(
                    f"rank {rank}: setValues called with non-local rows "
                    f"{rows_arr} outside ownership [{rstart}, {rend})"
                )
        return orig_setvalues(mat, rows, cols, values, *args, **kwargs)

    fdmod._mat_setvalues_local = set_values_checked
    try:
        P2, _extras2 = fdmod.build_sparse_fd_jacobian(
            ctx,
            np.asarray(u0, dtype=np.float64),
            eps=1.0e-8,
            drop_tol=0.0,
        ...
        fdmod._mat_setvalues_local = orig_setvalues
