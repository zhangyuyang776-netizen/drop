...
@pytest.mark.mpi
def test_p3_4_1_5_failfast_options_use_amat_mpi(tmp_path: Path, fs_type: str, monkeypatch):
    ...
    try:
        ...
        ksp.setOptionsPrefix("ff_")
        ksp.setOperators(A_shell, P_aij)
        ksp.setType("gmres")

        opts = PETSc.Options("ff_")
        try:
            opts.setValue("pc_use_amat", "1")
        except Exception:
            opts["pc_use_amat"] = "1"

        with pytest.raises(RuntimeError, match="pc_use_amat|use Amat|failfast"):
            apply_structured_pc(ksp, cfg, layout, A_shell, P_aij)
    finally:
