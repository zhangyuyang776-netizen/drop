...
@pytest.mark.mpi
def test_p3_4_5_regression_fieldsplit_shell_safe_mpi(tmp_path: Path, fs_type: str):
    ...
    try:
        ...
        for sksp in subksps:
            ...
            assert _is_aij(Ps_t)
            assert not _is_shell_like(Ps_t)
            if fs_type == "schur":
                assert _is_aij(As_t) or (As_t == "schurcomplement")
            else:
                assert _is_aij(As_t)
            assert not _is_shell_like(As_t)
