...
def test_residual_repeatability_mpi(tmp_path: Path):
    ...
    from assembly.residual_global import residual_petsc  # noqa: E402
    ...
    scatter_layout_to_local(ctx_local, u0, aXl_liq, aXl_gas, aXl_if, rank=rank)

    Xg = local_state_to_global(mgr, Xl_liq, Xl_gas, Xl_if)
    Fg1 = residual_petsc(mgr, ld, ctx, Xg)
    Fg2 = residual_petsc(mgr, ld, ctx, Xg)

    diff = Fg1.duplicate()
    Fg1.copy(diff)
    diff.axpy(-1.0, Fg2)

    norm_diff = float(diff.norm(PETSc.NormType.NORM_INFINITY))
    norm_ref = float(Fg1.norm(PETSc.NormType.NORM_INFINITY))

    assert norm_diff <= 1.0e-12 * max(1.0, norm_ref)
    try:
        comm.barrier()
    except Exception:
        pass
