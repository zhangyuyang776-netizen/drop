...
def test_sparse_fd_prealloc_lengths_match_dm_ownership(tmp_path: Path):
    ...
    cfg, layout, ctx, _u0 = _build_case(tmp_rank, comm.getSize(), rank)
    ...
    rstart, rend = Xg.getOwnershipRange()
    n_owned = int(rend - rstart)

    ownership_ranges = get_global_ownership_ranges_from_vec(Xg)
    ...
    indices = np.asarray(local_pattern.indices, dtype=np.int64)
    for i in range(int(local_pattern.rows_global.size)):
        start = int(indptr[i])
        end = int(indptr[i + 1])
        local_row_cols.append([int(c) for c in indices[start:end]])

    d_nz, o_nz = count_diag_off_nnz_for_local_rows(
        local_pattern.rows_global,
        local_row_cols,
        owner_map,
        rank,
        (int(rstart), int(rend)),
    ...
    assert d_nz.shape == (n_owned,)
    assert o_nz.shape == (n_owned,)
    assert np.all(d_nz >= 0)
