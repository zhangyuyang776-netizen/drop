...
def _build_case(tmp_path: Path, nproc: int, rank: int):
    ...
    cfg.nonlinear.backend = "petsc_parallel"
...
def test_petsc_snes_parallel_defaults_ignore_cfg_mpi(tmp_path: Path):
    ...
    cfg.petsc.snes_type = "ngmres"
    cfg.petsc.linesearch_type = "basic"
    cfg.petsc.ksp_type = "cg"
    cfg.petsc.rtol = 1.0e-2
    cfg.petsc.atol = 1.0e-3
    cfg.petsc.max_it = 5
    cfg.petsc.restart = 2
    ...
    extra = result.diag.extra or {}

    assert str(extra.get("snes_type", "")).lower() == "newtonls"
    assert str(extra.get("linesearch_type", "")).lower() == "l2"
    assert str(extra.get("ksp_type", "")).lower() == "gmres"
    assert str(extra.get("pc_type", "")).lower() == "asm"
    assert extra.get("jacobian_mode") == "mf"
...
def test_petsc_snes_parallel_vec_mat_pc_types_mpi(tmp_path: Path):
    _import_mpi4py_or_skip()
    PETSc = _import_petsc_or_skip()
    _import_chemistry_or_skip()
