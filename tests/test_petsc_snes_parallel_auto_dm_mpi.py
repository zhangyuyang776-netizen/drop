...
def test_petsc_snes_parallel_auto_dm_mpi(tmp_path: Path):
    ...
    _cfg, _layout, ctx, u0 = _build_case(tmp_rank, comm.getSize(), rank)
    ...
    assert "dm" in ctx.meta
    assert "dm_manager" in ctx.meta
    assert "layout_dist" in ctx.meta
