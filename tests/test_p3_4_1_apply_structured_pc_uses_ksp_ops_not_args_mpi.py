...
@pytest.mark.mpi
def test_p3_4_1_apply_structured_pc_uses_ksp_ops_not_args_mpi(tmp_path: Path, fs_type: str):
    ...
    try:
        ...
        diag_pc = apply_structured_pc(ksp, cfg, layout, P_aij, P_aij)
        ...
        assert uses_amat is False, "apply_structured_pc must disable amat based on ksp operators"
...
@pytest.mark.parametrize("fs_type", ["additive", "schur"])
@pytest.mark.mpi
def test_p3_4_1_4_wrong_args_do_not_mutate_ksp_ops_mpi(tmp_path: Path, fs_type: str):
    watchdog_prev = os.environ.get("DROPLET_TEST_WATCHDOG")
    os.environ["DROPLET_TEST_WATCHDOG"] = "1"
    try:
        _import_mpi4py_or_skip()
        ...
        A_before, P_before = ksp.getOperators()
        ...
        A_after, P_after = ksp.getOperators()
        try:
            assert A_after.handle == A_before.handle
            assert P_after.handle == P_before.handle
        except Exception:
            assert A_after is A_before
        ...
        try:
            ...
            uses_amat = diag_pc.get("uses_amat", None)
        assert uses_amat is False, "fieldsplit must disable amat even with wrong args"

        ksp.setUp()

        A_post, P_post = ksp.getOperators()
        try:
            assert A_post.handle == A_before.handle
            assert P_post.handle == P_before.handle
        except Exception:
            assert A_post is A_before
            assert P_post is P_before
