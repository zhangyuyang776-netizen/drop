...
def test_fieldsplit_additive_is_covers_owned_rows_mpi(tmp_path: Path):
    ...
    assert diag_pc.get("enabled", False) is True
    assert diag_pc.get("fieldsplit_type") == "additive"
    assert diag_pc.get("fieldsplit", {}).get("plan") == "bulk_iface"

    split_names = diag_pc.get("split_names", []) or []
    assert "bulk" in split_names
    assert "iface" in split_names

    split_sizes_local = diag_pc.get("split_sizes_local", {}) or {}
    assert "bulk" in split_sizes_local
    assert "iface" in split_sizes_local
    bulk_n = int(split_sizes_local["bulk"])
    iface_n = int(split_sizes_local["iface"])
