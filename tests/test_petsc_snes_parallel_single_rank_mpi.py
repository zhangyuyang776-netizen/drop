...
def _build_case(tmp_path: Path, rank: int):
    ...
    cfg.geometry.N_liq = 4
    cfg.geometry.N_gas = 8
    ...
    cfg.nonlinear.max_outer_iter = 1
...
def test_petsc_snes_parallel_single_rank_mpi(tmp_path: Path):
    ...
    if comm.getSize() != 1:
        pytest.skip("Single-rank MPI test: run with mpiexec -n 1 ...")

    rank = comm.getRank()
    tmp_rank = tmp_path / f"rank_{rank:03d}"
    tmp_rank.mkdir(parents=True, exist_ok=True)

    cfg, layout, ctx, u0 = _build_case(tmp_rank, rank)
    ...
    assert int(extra.get("comm_size", -1)) == 1
