...
def test_sparse_fd_pattern_rows_are_local(tmp_path: Path):
    ...
    try:
        ...
        P_dm = None
    P, extras = build_sparse_fd_jacobian(
        ctx,
    ...
    pattern_local = extras.get("pattern_local")
    if pattern_local is None:
        pytest.skip("pattern_local not available in stats.")

    rows_global = np.asarray(pattern_local.rows_global, dtype=np.int64)
    if rows_global.size == 0:
        return

    assert np.all(rows_global >= rstart), (
        f"rank {rank}: pattern rows contain entries < rstart={rstart}"
    )
    assert np.all(rows_global < rend), (
        f"rank {rank}: pattern rows contain entries >= rend={rend}"
    )
    assert rows_global.size <= n_owned, (
        f"rank {rank}: pattern rows size {rows_global.size} > owned {n_owned}"
    )
