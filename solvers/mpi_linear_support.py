...
def validate_mpi_linear_support(cfg: Any) -> None:
    ...
    if fs_type == FieldSplitType.SCHUR.value:
        if raw_subsolvers_pre is not None:
            raise ValueError(
                "MPI linear support: fieldsplit.type=schur forbids 'subsolvers' overrides in this stage. "
                "Remove linear.fieldsplit.subsolvers for schur."
            )
